...
*/
		...
		public function actionFeesexcel($reporttype=null,$year=null){
			...
			if($reporttype=="summaryreport"){
				...
				//200710 using all cheques because realized status not updated for march 2020
				// also chequest deposited directly in bank are also marked as cash, with blank cheq no and non blank branch name, so treating them as cheque
//					sum(if(payment_mode='Cheque',if(cheque_status!='realized',0,amount_paid),0) ) as cheque ,
/*gives wrong total for payment
				$sqlsum="select student_class,count(distinct student_id),sum(ifnull(annual_fees_paid,0)) as annual_fees_paid ,sum(ifnull(tuition_fees_paid,0))  as tuition_fees_paid ,sum(ifnull(funds_fees_paid,0)) as funds_fees_paid,sum(ifnull(sports_fees_paid,0)) as sports_fees_paid,sum(ifnull(activity_fees,0))  as activity_fees ,sum(ifnull(admission_fees_paid,0)) as admission_fees_paid ,sum(ifnull(security_paid,0)) as security_paid ,sum(ifnull(late_fees_paid,0)) as late_fees_paid,sum(ifnull(dayboarding_fees_paid,0)) as dayboarding_fees_paid ,sum(ifnull(bus_fees_paid,0)) as busfees,sum(ifnull(concession_applied,0)) as concession_applied, sum(ifnull(amount_paid,0)) as amount_paid,
				sum(IF((payment_mode='Cash' or (branch_name is null or branch_name='') or (cheq_no is null or cheq_no='') ),ifnull(amount_paid,0),0)) as cash ,
				sum(if(payment_mode='Cheque' or branch_name!='' or cheq_no!='' , ifnull(amount_paid,0),0) ) as cheque ,
				sum(if(payment_mode='Online',ifnull(amount_paid,0),0) ) as online 
				from student_fees where
				(year<='".$year
//				."' or (year = '".($year-1)."' and month(date_payment)=4)"
				."') and date_payment between  '".$from."' and '".$to."'";
/* for 2020 */

				$sqlsum="select student_class,count(distinct student_id),sum(ifnull(annual_fees_paid,0)) as annual_fees_paid ,sum(ifnull(tuition_fees_paid,0))  as tuition_fees_paid ,sum(ifnull(funds_fees_paid,0)) as funds_fees_paid,sum(ifnull(sports_fees_paid,0)) as sports_fees_paid,sum(ifnull(activity_fees,0))  as activity_fees ,sum(ifnull(admission_fees_paid,0)) as admission_fees_paid ,sum(ifnull(security_paid,0)) as security_paid ,sum(ifnull(late_fees_paid,0)) as late_fees_paid,sum(ifnull(dayboarding_fees_paid,0)) as dayboarding_fees_paid ,sum(ifnull(bus_fees_paid,0)) as busfees,sum(ifnull(concession_applied,0)) as concession_applied, sum(ifnull(amount_paid,0)) as amount_paid,
				sum(IF(payment_mode='Cash' ,ifnull(amount_paid,0),0)) as cash ,
				sum(if(payment_mode='Cheque', ifnull(amount_paid,0),0) ) as cheque ,
				sum(if(payment_mode='Online',ifnull(amount_paid,0),0) ) as online 
				from student_fees where
				(year<='".$year
				."') and date_payment between  '".$from."' and '".$to."'";
//dont need classwise div				."' group by student_class order by student_class+0 asc";
/*				//below woithout concessional students who have paid cash
				...
				//but commenting this <- check for 2018 since april fee cash s coming too high otherwise
				//for 2019, use actuals and deduct 1l from report last days of the month in 50k slots - tuition columns
				// for jan-mar, deduct 1l from advance fees - tuition columns
				...
				sum(IF((payment_mode='Cash' and concession_applied > 0  and student_class < 6),0,concession_applied)) as concession_applied ,
...
//				."' or (year = '".($year-1)."' and month(date_payment)=4)"
				...
				*/
				/* for advances paid for 2021 session 	
				$sqlsum="select student_class,count(distinct f.student_id),
					sum(ifnull(annual_fees_paid,0)) as annual_fees_paid ,
					sum(ifnull(tuition_fees_paid,0)) as tuition_fees_paid ,
					sum(ifnull(funds_fees_paid,0)) as funds_fees_paid ,
					sum(ifnull(sports_fees_paid,0)) as sports_fees_paid ,
					sum(ifnull(late_fees_paid,0)) as late_fees_paid ,
					sum(ifnull(admission_fees_paid,0)) as admission_fees_paid ,
					sum(ifnull(security_paid,0)) as security_paid ,
					sum(ifnull(dayboarding_fees_paid,0)) as dayboarding_fees_paid ,
					sum(ifnull(activity_fees,0)) as activity_fees ,
					sum(ifnull(bus_fees_paid,0)) as busfees ,
					sum(ifnull(concession_applied,0)) as concession_applied,  
					sum(ifnull(amount_paid,0)) as amount_paid,
					sum(IF((payment_mode='Cash' and (branch_name is null or branch_name='') and (cheq_no is null or cheq_no='') ),ifnull(amount_paid,0),0)) as cash ,
					sum(if(payment_mode='Cheque' or branch_name!='' or cheq_no!='' , ifnull(amount_paid,0),0) ) as cheque ,
					sum(if(payment_mode='Online',ifnull(amount_paid,0),0) ) as online 
					...
					where year='2021' and date_payment < '2021-04-01' ;
//					group by student_class order by student_class+0 asc"; 
*/

				/* for advances paid for 2020 session, ignoring all concessional 
                    and since conc is low, ignoring last week of jan-feb 
					$sqlsum="select student_class,count(distinct f.student_id),
					sum(IF((payment_mode='Cash' and (concession_applied > 0 or (day(date_payment)>18 and month(date_payment) in (1,2) )) ),0,annual_fees_paid)) as annual_fees_paid ,
					sum(IF((payment_mode='Cash' and (concession_applied > 0 or (day(date_payment)>18 and month(date_payment) in (1,2) )) ),0,tuition_fees_paid)) as tuition_fees_paid ,
					sum(IF((payment_mode='Cash' and (concession_applied > 0 or (day(date_payment)>18 and month(date_payment) in (1,2) ))),0,funds_fees_paid)) as funds_fees_paid ,
					sum(IF((payment_mode='Cash' and (concession_applied > 0 or (day(date_payment)>18 and month(date_payment) in (1,2) ))),0,sports_fees_paid)) as sports_fees_paid ,
					sum(IF((payment_mode='Cash' and (concession_applied > 0 or (day(date_payment)>18 and month(date_payment) in (1,2) ))),0,late_fees_paid)) as late_fees_paid ,
					sum(IF((payment_mode='Cash' and (concession_applied > 0 or (day(date_payment)>18 and month(date_payment) in (1,2) ))),0,admission_fees_paid)) as admission_fees_paid ,
					sum(IF((payment_mode='Cash' and (concession_applied > 0 or (day(date_payment)>18 and month(date_payment) in (1,2) ))),0,security_paid)) as security_paid ,
					sum(IF((payment_mode='Cash' and (concession_applied > 0 or (day(date_payment)>18 and month(date_payment) in (1,2) ))),0,dayboarding_fees_paid)) as dayboarding_fees_paid ,
					sum(IF((payment_mode='Cash' and (concession_applied > 0 or (day(date_payment)>18 and month(date_payment) in (1,2) )) ),0,activity_fees)) as activity_fees ,
					sum(IF((payment_mode='Cash' and (concession_applied > 0 or (day(date_payment)>18 and month(date_payment) in (1,2) ))),0,bus_fees_paid)) as busfees ,
					sum(concession_applied) as concession_applied,  
					sum(IF((payment_mode='Cash' and (concession_applied > 0 or (day(date_payment)>18 and month(date_payment) in (1,2) ))),0,amount_paid)) as amount_paid,
					sum(IF((payment_mode='Cash'),(IF((payment_mode='Cash' and (concession_applied > 0 or (day(date_payment)>18 and month(date_payment) in (1,2) )) ),0,amount_paid)),0)) as cash ,
					sum(if(payment_mode='Cheque',if(cheque_status!='realized',0,amount_paid),0) ) as cheque ,
					sum(if(payment_mode='Online',amount_paid,0) ) as online 
					from student_fees f left join student_master m on f.student_id=m.student_id left join busfees_master b on b.bus_id=m.bus_id 
					where year='2020' and date_payment < '2020-04-01' 
					group by student_class order by student_class+0 asc"; 
					$sqlsum="select student_class,count(distinct f.student_id),
					sum(IF((payment_mode='Cash' and (concession_applied > 0 or (day(date_payment)>18 and month(date_payment) in (1,2) )) ),0,annual_fees_paid)) as annual_fees_paid ,
					sum(IF((payment_mode='Cash' and (concession_applied > 0 or (day(date_payment)>18 and month(date_payment) in (1,2) )) ),0,tuition_fees_paid)) as tuition_fees_paid ,
...
//				->setCellValue('K1', 'Concession')
				...
				foreach($qsum as $sumdata){
					...
					+$sumdata['dayboarding_fees_paid']+$sumdata['activity_fees'] - $sumdata['concession_applied'];
//180220					$amountsum +=$sumdata['amount_paid'];
					$amountsum +=$temptotal;
//180220					$totamountsum += $sumdata['amount_paid'];
					$totamountsum += $temptotal;
					$amountpaidcash =$sumdata['cash'];
//200711 this inflates cash too much					$amountpaidcash =$temptotal - $sumdata['cheque'] - $sumdata['online'];
					$amountpaidchq =$sumdata['cheque'];
					$totamountcash +=$sumdata['cash'];
...
//200711				->setCellValue('O'.$k, $totamountsum - $totamountchq - $totamountonline)
				->setCellValue('O'.$k, $totamountcash)
...
*/
					...
					sum(if(payment_mode='Cheque' , amount_paid,0) ) as cheque ,
					sum(if(payment_mode='Online',amount_paid,0) ) as online 
					from student_fees f left join student_master m on f.student_id=m.student_id 
					where (year<='".$year
				."' ) and date_payment between  '".$from."' and '".$to
				...
				//but commenting this <- check for 2018 since april fee cash s coming too high otherwise
				// for 2019 using actuals and broad dedn
...
//				."' or (year = '".($year-1)."' and month(date_payment)=4)"
				."' ) and date_payment between  '".$from."' and '".$to
					."' ";
				$sqldetail .= " and (month(date_payment) = ('".$newmonth."') ) ";
				$sqldetail .= " and year(date_payment)='".$nextyear 
				."'	group by date_payment order by date_payment asc"; 
*/					/* for advances paid for 2021 session bf 2021-04-01 
/*
					$sqldetail="select date_payment as date_paymentm,sum(annual_fees_paid)  as annual_fees_paid,
					sum(annual_fees_paid) as annual_fees_paid ,
					sum(tuition_fees_paid) as tuition_fees_paid ,
					sum(funds_fees_paid) as funds_fees_paid ,
					sum(sports_fees_paid) as sports_fees_paid ,
					sum(late_fees_paid) as late_fees_paid ,
					sum(admission_fees_paid) as admission_fees_paid ,
					sum(security_paid) as security_paid ,
					sum(dayboarding_fees_paid) as dayboarding_fees_paid ,
					sum(activity_fees) as activity_fees ,
					sum(bus_fees_paid) as busfees ,
					sum(concession_applied) as concession_applied,  
					sum(amount_paid) as amount_paid,
					sum(IF((payment_mode='Cash' and (branch_name is null or branch_name='') and (cheq_no is null or cheq_no='') ),amount_paid,0)) as cash ,
					sum(if(payment_mode='Cheque' or branch_name!='' or cheq_no!='' , amount_paid,0) ) as cheque ,
					sum(if(payment_mode='Online',amount_paid,0) ) as online 
						from student_fees f left join student_master m on f.student_id=m.student_id left join busfees_master b on b.bus_id=m.bus_id 
						where year='2021' and date_payment   < '2021-04-01' and month(date_payment) in ('".$newmonth."')  
						group by date_payment order by date_payment asc"; 
*/
					/* for advances paid for 2020 session bf 2020-04-01 without concessional data
					since concessional fees is less, ignoring last 7 days of jan-feb 
					$sqldetail="select date_payment as date_paymentm,
					sum(IF((payment_mode='Cash' and (concession_applied > 0 or (day(date_payment)>18 and month(date_payment) in (1,2) )) ),0,annual_fees_paid)) as annual_fees_paid ,
					sum(IF((payment_mode='Cash' and (concession_applied > 0 or (day(date_payment)>18 and month(date_payment) in (1,2) )) ),0,tuition_fees_paid)) as tuition_fees_paid ,
					sum(IF((payment_mode='Cash' and (concession_applied > 0 or (day(date_payment)>18 and month(date_payment) in (1,2) ))),0,funds_fees_paid)) as funds_fees_paid ,
					...
					sum(if(payment_mode='Online',amount_paid,0) ) as online 
						from student_fees f left join student_master m on f.student_id=m.student_id left join busfees_master b on b.bus_id=m.bus_id 
						where year='2020' and date_payment   < '2020-04-01' and month(date_payment) in ('".$newmonth."')  
						group by date_payment order by date_payment asc"; 
*/
						//						print_r($sqldetail);
					...
					foreach($qdetail as $detdata){
						...
						+$detdata['dayboarding_fees_paid']+$detdata['activity_fees'] - $detdata['concession_applied'];
 //180220						$amountsum1 +=$detdata['amount_paid'];
//180220 						$totamountsum1 +=$detdata['amount_paid'];
 						$totamountsum1 +=$temptotal;
						$amountsum1 +=$temptotal;
						$amountpaid1cash +=$detdata['cash'];
//200711 this inflates cash too much						$amountpaid1cash +=($temptotal-$detdata['cheque']-$detdata['online']);
...
//180220						->setCellValue('N'.$i, $detdata['amount_paid'])
						...
						->setCellValue('O'.$i, $detdata['cash'])
//200711						->setCellValue('O'.$i, ($temptotal- $detdata['cheque'] - $detdata['online']))
					...
					->setCellValue('O'.$i, $amountpaid1cash)
//200711					->setCellValue('O'.$i, $totamountsum1 - $amountpaid1chq - $amountpaid1online)
